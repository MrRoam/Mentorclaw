...
import { OpenClawTurnBridge, type OpenClawTurnBridgeLike } from "../integration/openclaw-turn-bridge.ts";
import type { EducationSnapshot, SchedulePreferences } from "../schemas/education.ts";
import type {
  LearnerSummary,
  LearningEvent,
  PlanCreationInput,
...
export class DebugUiService {
  ...
  readonly turnBridge: OpenClawTurnBridgeLike;

  constructor(runtimeRoot: string, options?: { turnBridge?: OpenClawTurnBridgeLike }) {
    ...
    this.turnBridge = options?.turnBridge ?? new OpenClawTurnBridge();
  }
  ...
  async getSnapshot(): Promise<DebugDashboardSnapshot> {
    const [validation, learner, index, sessionBindings, learnerEvents, planIds, education] = await Promise.all([
  ...
  async updateSchedulePreferences(preferences: Partial<SchedulePreferences>): Promise<DebugDashboardSnapshot> {
    await this.educationRepo.updateSchedulePreferences(preferences);
  ...
  async bindSession(request: BindSessionRequest): Promise<DebugDashboardSnapshot> {
    const rawSessionRef = request.sessionKey.trim();
    if (!rawSessionRef) {
      throw new Error("Session key is required.");
    }
    const session = await this.turnBridge.resolveSessionHandle(this.runtimeRoot, rawSessionRef);
  ...
  async unbindSession(sessionKey: string): Promise<DebugDashboardSnapshot> {
    const session = await this.turnBridge.resolveSessionHandle(this.runtimeRoot, sessionKey.trim());
    await this.bindingStore.remove(session.sessionKey);
    return this.getSnapshot();
  }

  async handleUserTurn(request: HandleTurnRequest): Promise<DebugTurnResponse> {
    const startedAt = Date.now();
    const sessionRef = request.sessionKey.trim() || browserSessionKey("browser");
    const message = request.message.trim();
    if (!message) {
      throw new Error("Message is required.");
    }

    const session = await this.turnBridge.resolveSessionHandle(this.runtimeRoot, sessionRef);
    const bound = await this.bindingStore.get(session.sessionKey);
    const planId = request.planId ?? bound?.planId;
    const threadId =
      request.threadId ?? (request.planId && bound?.planId !== request.planId ? null : (bound?.threadId ?? null));
    ...
    const pendingBinding =
      planId || bound
        ? {
            sessionKey: session.sessionKey,
            planId: planId ?? bound!.planId,
            threadId,
            updatedAt: nowIso(),
            lastWorkflow: request.forceWorkflow ?? bound?.lastWorkflow ?? "pending-live-turn",
            pendingSignals: request.forceWorkflow ? { forceWorkflow: request.forceWorkflow } : undefined,
          }
        : null;
    if (pendingBinding) {
      await this.bindingStore.set(pendingBinding);
    }
    ...
    const bridgeCompletedAt = Date.now();

    const binding = await this.bindingStore.get(liveTurn.sessionKey);
    if (!binding?.threadId) {
      throw new Error("OpenClaw turn completed, but mentorclaw did not resolve a concrete plan/thread binding.");
    }
    ...
    return {
      ...
      liveTurn: {
        ...
        agentId: liveTurn.agentId,
        durationMs: liveTurn.durationMs,
        stopReason: liveTurn.stopReason,
        workspaceDir: liveTurn.workspaceDir,
      },
      timing: {
        totalMs: completedAt - startedAt,
        bridgeMs: bridgeCompletedAt - bridgeStartedAt,
        snapshotMs: completedAt - snapshotStartedAt,
        openclawMs: liveTurn.durationMs,
      },
      snapshot,
    };
  }
  ...
  async recordAssistantReply(request: RecordAssistantReplyRequest): Promise<DebugDashboardSnapshot> {
    ...
    const session = await this.turnBridge.resolveSessionHandle(this.runtimeRoot, rawSessionRef);
    const binding = await this.bindingStore.get(session.sessionKey);
    if (!binding) {
      throw new Error(`No binding found for session ${session.sessionKey}.`);
    }
    if (!binding.threadId) {
      throw new Error(`Session ${session.sessionKey} is not yet attached to a concrete thread.`);
    }
  ...
  private runtimeLocalFiles(): DebugLocalFileRef[] {
    return [
      this.makeFileRef(
        this.educationRepo.files.connections,
        "Course Connections",
        "runtime",
        "Stored source connections for timetable, resources, and replay integrations.",
      ),
      this.makeFileRef(
        this.educationRepo.files.courses,
        "Courses",
        "runtime",
        "Stable course records used by schedule and future resource linking.",
      ),
      this.makeFileRef(
        this.educationRepo.files.courseItems,
        "Course Items",
        "runtime",
        "Class sessions, exams, assignments, notices, and replay entries.",
      ),
      this.makeFileRef(
        this.educationRepo.files.courseResources,
        "Course Resources",
        "runtime",
        "Course and replay-linked resources such as subtitles, PPTs, and videos.",
      ),
      this.makeFileRef(
        this.educationRepo.files.schedulePreferences,
        "Schedule Preferences",
        "runtime",
        "Persisted schedule view mode and timetable visibility toggle.",
      ),
      this.makeFileRef(
